...
void main()
{
	...
	float timeDependentInstanceXValue = mod(instanceX + velocityX,  1.0);
    
    float instanceY =  floor(gl_InstanceID/iCount);
    
    vPos.x += timeDependentInstanceXValue * uWidth ;
    vPos.y += instanceY * uVspacing;
    
    //noise-------------------------------------
    
//    float nzx = snoise (vec2(vPos.x*uYnoiseFreq , (instanceY+1)*uYnoiseFreq* timeValue_b * 0.0001)) * uYnoiseAmp*.5 - uYnoiseAmp*.5*.5;
//    float nzy = snoise (vec2(vPos.x*uYnoiseFreq , (instanceY+1)*uYnoiseFreq* timeValue_b * 0.0001)) * uYnoiseAmp - uYnoiseAmp*.5;
//    float nzz = snoise (vec2(vPos.x*uZnoiseFreq , instanceY*uZnoiseFreq* timeValue_b *0.0008)) * uZnoiseAmp + uZnoiseAmp*.5;
